...
import { ArrowRight, Check, Sparkles, Folder, Tag, Plus, Mail, Palette } from 'lucide-react';
import { suggestCategories, saveConfiguration } from './actions';
...
export default function EmailSorterPage() {
  ...
  const handleNext = async () => {
    if (step === 1) {
      ...
      if (aiCats.length === 0) {
        const res = await suggestCategories(manifesto, []);
        if (res.success) setAiCats(res.data);
        else setError('AI Error: ' + res.error);
      }
      setLoadingAi(false);
      setStep(2);
    } else if (step === 2) {
  ...
  const fetchMore = async () => {
    setLoadingAi(true);
    const res = await suggestCategories(manifesto, [...aiCats.map(c => c.id), ...DEFAULTS.map(c => c.id)]);
  ...
  const updateColor = (color: string) => {
    ...
    // Note: Defaults are const, but in a real app we'd copy them to state to edit colors
  ...
  const handleConnect = async () => {
    ...
    const config = {
      manifesto,
      categories: [...DEFAULTS, ...aiCats].filter(c => selectedIds.has(c.id)),
      timing: { type: timingType, interval: batchInterval, time: dailyTime }
    };
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 max-w-2xl mx-auto w-full">
        ...
        {step === 1 && (
          <div className="w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <textarea 
              ...
              className="w-full h-48 bg-gray-50 border border-gray-200 rounded-2xl p-6 text-lg focus:ring-2 focus:ring-black focus:border-transparent outline-none resize-none shadow-sm placeholder-gray-400 transition-all" 
            ...
            <div className="mt-8 flex justify-end">
              <button onClick={handleNext} disabled={loadingAi} className="bg-black text-white px-8 py-4 rounded-full font-medium hover:bg-gray-800 transition flex items-center gap-2 text-lg shadow-xl hover:shadow-2xl disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          <div className="w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
             ...
             <div className="space-y-4 mt-8">
                ...
                <label className={`flex items-center gap-5 p-6 border rounded-2xl cursor-pointer transition-all ${timingType === 'realtime' ? 'border-black bg-gray-50' : 'border-gray-200 hover:border-gray-300'}`}>
                  ...
                  <div>
                    <div className="font-medium text-lg">Real-time</div>
                    <div className="text-gray-500">Sorts immediately as emails arrive.</div>
                  </div>
                </label>
                ...
                <div className={`border rounded-2xl transition-all ${timingType === 'hourly' ? 'border-black bg-gray-50' : 'border-gray-200 hover:border-gray-300'}`}>
                   <label className="flex items-center gap-5 p-6 cursor-pointer">
                      ...
                      <div>
                        <div className="font-medium text-lg">Hourly Batch</div>
                        <div className="text-gray-500">Process inbox periodically.</div>
                      </div>
                   </label>
                   {timingType === 'hourly' && (
                ...
                <div className={`border rounded-2xl transition-all ${timingType === 'daily' ? 'border-black bg-gray-50' : 'border-gray-200 hover:border-gray-300'}`}>
                   <label className="flex items-center gap-5 p-6 cursor-pointer">
                      ...
                      <div>
                        <div className="font-medium text-lg">Daily Cleanse</div>
                        <div className="text-gray-500">Wake up to a sorted inbox.</div>
                      </div>
                   </label>
                   {timingType === 'daily' && (
        ...
        {step === 4 && (
          <div className="text-center animate-in fade-in zoom-in-95 duration-500">
            ...
            <p className="text-gray-500 mb-10 max-w-sm mx-auto text-lg">We need permission to manage your labels and move messages. We never store email content.</p>
        ...
        {step === 5 && (
          <div className="text-center animate-in fade-in zoom-in-95 duration-500">
             ...
             <p className="text-gray-500 text-lg">The sorter is running in the background.</p>
             <div className="mt-8 p-6 bg-gray-50 rounded-2xl border border-gray-200 text-sm text-gray-500 max-w-md mx-auto">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-2xl border border-gray-100 w-80 grid grid-cols-5 gap-3" onClick={e => e.stopPropagation()}>
                {COLORS.map(c => (
                    <button 
                        ...
                        className="w-10 h-10 rounded-full hover:scale-110 transition border-2 border-transparent hover:border-black/10 shadow-sm"
...
function CategoryCard({ cat, isSelected, onToggle, onColorClick, small }: { cat: Category, isSelected: boolean, onToggle: () => void, onColorClick: () => void, small?: boolean }) {
  return (
    <div 
        onClick={onToggle}
        className={`
            border rounded-2xl flex items-center justify-between cursor-pointer transition-all duration-200
            ${isSelected ? 'border-black bg-gray-50 shadow-md' : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50/50'}
            ${small ? 'py-3 px-4' : 'py-5 px-5'}
        `}
    >
        <div className="flex items-center gap-4">
            <button 
                onClick={(e) => { e.stopPropagation(); onColorClick(); }}
                className="w-10 h-10 rounded-full border border-gray-200 shadow-sm flex items-center justify-center hover:scale-110 transition group"
        ...
        <div className={`
            ...
            ${isSelected ? 'bg-black border-black text-white scale-100' : 'border-gray-200 text-transparent scale-90'}
        `}>
