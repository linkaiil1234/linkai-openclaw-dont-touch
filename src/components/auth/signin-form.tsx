...
export default function SignUpForm() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Fetch user data to create in MongoDB (via /auth/login endpoint)
      await fetchUserData();
      ...
      const errorCode = error?.code as TFirebaseErrorCodes;
      const errorMessage =
        FIREBASE_ERROR_CODES_MESSAGES[errorCode] ||
        error?.message ||
        ...
        <div>
          ...
          <div>
            <div className="w-full">
              <button
                className="inline-flex items-center justify-center gap-3 py-3 text-sm font-normal transition-colors bg-muted rounded-lg hover:bg-accent/30 border w-full cursor-pointer"
